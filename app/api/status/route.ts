...
export async function GET() {
  try {
    ...
    const statusData = await getCachedAnalytics(
      cacheKey,
...
async function fetchSystemStatus() {
  ...
  const [runStatsResult, scheduleStats, mappingStats] = await Promise.all([
    ...
    db.collection('api_metadata').countDocuments({ type: 'mapping' })
  ]);
  ...
  const successRate = recentStats.totalRuns > 0
    ? Math.round((recentStats.successfulRuns / recentStats.totalRuns) * 100)
    : 0;
  ...
  const dataVolumeToday = await db.collection('api_data').countDocuments({
