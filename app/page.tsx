...
async function getSystemStatus() {
  ...
  return {
    ...
    runs: { total: 0, running: 0, last24h: 0, successRate: 0 },
    schedules: { total: 0, active: 0 },
    mappings: { total: 0 },
    activity: { totalRuns: 0, successfulRuns: 0, failedRuns: 0, successRate: 0 }
  }
}
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <header className="mb-12">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold tracking-tight text-balance">Data Platform</h1>
              {/* <p className="mt-2 text-lg text-muted-foreground text-pretty">
                Connect any API without writing code. Configure, schedule, and automate data extraction.
              </p> */}
            </div>
        ...
        <div className="grid gap-4 md:grid-cols-4 mb-8">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{status.connections?.active || 0}</div>
              <p className="text-xs text-muted-foreground">
                {status.connections?.total || 0} total configured
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{status.schedules?.active || 0}</div>
              <p className="text-xs text-muted-foreground">
                {status.schedules?.total || 0} total schedules
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{status.runs?.total || 0}</div>
              <p className="text-xs text-muted-foreground">
                {status.runs?.last24h || 0} in last 24h
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{status.activity?.successRate || 0}%</div>
              <p className="text-xs text-muted-foreground">
                {status.activity?.successfulRuns || 0} successful runs
              </p>
            </CardContent>
          </Card>
        </div>
