...
import { NotFoundError, ValidationError, formatErrorResponse } from "@/lib/error-handler"
...
async function findConnectionById(db: any, connectionId: string) {
  ...
  try {
    ...
    const connection = await db.collection(CollectionManager.getCollectionName("METADATA")).findOne({
      type: "connection",
      $or: [{ "connectionData.name": connectionId }, { "connectionData.connectionId": connectionId }],
    })
    return connection
  }
}
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await db
      ...
      .updateOne({ type: "connection", _id: connectionObjectId }, updateDoc)
