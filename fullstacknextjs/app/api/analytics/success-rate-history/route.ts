...
import { cacheAside, CacheTTL } from "@/lib/cache-manager"
...
export async function GET(request: Request) {
  try {
    ...
    const analyticsData = await cacheAside(
      cacheKey,
  ...
  const pipeline = [
    { $match: matchConditions },
    {
      $group: {
        ...
        successfulRuns: {
          $sum: {
            $cond: [{ $eq: ["$runMetadata.status", "success"] }, 1, 0],
          },
        },
        failedRuns: {
          $sum: {
            $cond: [{ $eq: ["$runMetadata.status", "failed"] }, 1, 0],
          },
        },
      },
    },
    {
      $project: {
        ...
        failedRuns: 1,
        successRate: {
          $cond: [{ $gt: ["$totalRuns", 0] }, { $multiply: [{ $divide: ["$successfulRuns", "$totalRuns"] }, 100] }, 0],
        },
      },
    },
    { $sort: { date: 1 } },
