...
export default function ConnectionsPage() {
  ...
  const deleteConnection = async (connectionId: string, deleteData = false) => {
      ...
      {loading ? (
        ...
        <div className="grid gap-4">
          {connections.map((connection) => {
            const lastRun = connection.lastRun ? new Date(connection.lastRun).toLocaleString() : "Never"
                ...
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground mt-1 font-mono">ID: {connection.connectionId}</p>
                    </div>
                              ...
                              <div className="space-y-3">
                                ...
                                {deleteWithData ? (
                                  <span className="text-sm text-destructive">
                                    ⚠️ Warning: This will permanently delete all data collected by this connection. This
                                    action cannot be undone.
                                  </span>
                                ) : (
                                  <span className="text-sm text-muted-foreground">
                                    Note: Connection configuration will be deleted, but collected data will remain in
                                    the database.
                                  </span>
                                )}
                              </div>
