...
interface SuccessRateHistoryData {
  data?: Array<{
    date: string
    successRate: number
  }>
...
export default function MonitoringPage() {
  ...
  const [successRateHistoryData, setSuccessRateHistoryData] = useState<SuccessRateHistoryData | null>(null)
  const [logs, setLogs] = useState<Log[]>([])
  const [alerts, setAlerts] = useState<Alert[]>([])

  const [successRateData, setSuccessRateData] = useState<ChartDataPoint[]>([])
  ...
  const fetchMonitoringData = async () => {
    try {
      ...
      const [health, status, analytics, successRateHistory] = await Promise.all([
        ...
        successRateHistoryRes.json() as Promise<SuccessRateHistoryData>,
      ])
  ...
  const generateChartData = (
    ...
    successRateHistory: SuccessRateHistoryData,
  ) => {
    const now = new Date()

    if (successRateHistory?.data) {
      const successRateChart: ChartDataPoint[] = successRateHistory.data.map((day) => ({
      ...
      const successRateChart: ChartDataPoint[] = []
      for (let i = 5; i >= 0; i--) {
    ...
    const requestVolumeChart: ChartDataPoint[] = []
    for (let i = 5; i >= 0; i--) {
      const time = new Date(now.getTime() - i * 4 * 60 * 60 * 1000)
    ...
    const responseTimeChart: ChartDataPoint[] = []
    for (let i = 5; i >= 0; i--) {
      const time = new Date(now.getTime() - i * 4 * 60 * 60 * 1000)
  ...
  const generateMockLogsAndAlerts = (status: StatusData) => {
    const mockLogs: Log[] = [
      {
        id: 1,
        level: "info",
      ...
      {
        id: 2,
        level: (status.performance?.avgResponseTime || 0) > 300 ? "warning" : "info",
        connection: status.topConnections?.[0]?.connectionId || "API Connection",
      ...
      {
        id: 3,
        level: (status.activity?.failedRuns || 0) > 0 ? "error" : "info",
        connection: status.topConnections?.[1]?.connectionId || "Weather API",
        message:
          (status.activity?.failedRuns || 0) > 0 ? "Connection timeout detected" : "Data transformation completed",
    ...
    if ((status.activity?.failedRuns || 0) > 0) {
      mockAlerts.push({
        id: 1,
        severity: "high",
        connection: status.topConnections?.[1]?.connectionId || "Weather API",
    ...
    if ((status.performance?.avgResponseTime || 0) > 300) {
      mockAlerts.push({
        id: 2,
        severity: "medium",
        connection: status.topConnections?.[0]?.connectionId || "API Connection",
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-1">
                {loading
                  ? "Loading system health and performance metrics..."
        ...
        {error && (
          <Card className="mb-6 border-destructive">
            <CardContent className="pt-6">
              ...
              <button onClick={fetchMonitoringData} className="mt-2 text-sm text-primary hover:underline">
                Try again
              </button>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {loading ? (
                  ...
                  `${statusData?.performance?.avgResponseTime || 245}ms`
                )}
              </div>
              <div className="flex items-center text-xs text-green-600 mt-1">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="flex items-center text-xs text-yellow-600 mt-1">
                ...
                {alerts.filter((a: any) => a.severity === "high").length} high,{" "}
                {alerts.filter((a: any) => a.severity === "medium").length} medium
              </div>
            </CardContent>
          </Card>
        </div>
                  ...
                  {statusData?.topConnections?.slice(0, 2).map((conn: any, index: number) => (
                    <div key={conn.connectionId} className="p-4 border rounded-lg">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <Badge
                          variant={
                            conn.successRate > 95 ? "default" : conn.successRate > 80 ? "secondary" : "destructive"
                          }
                          className="gap-1"
                        >
                          {conn.successRate > 95 ? (
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              Run some API connections to see monitoring data
                            </p>
                          </div>
                        </div>
                      </div>
