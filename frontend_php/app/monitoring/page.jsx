...
export default function MonitoringPage() {
  ...
  const generateChartData = (status, successRateHistory) => {
    ...
    if (successRateHistory?.data) {
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        successRateChart.push({ time: time.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }), rate: Math.max(90, Math.min(100, successRate + (Math.random() - 0.5) * 10)) })
      }
      setSuccessRateData(successRateChart)
    }
    ...
    for (let i = 5; i >= 0; i--) {
      const time = new Date(now.getTime() - i * 4 * 60 * 60 * 1000)
      requestVolumeChart.push({ time: time.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }), requests: Math.floor(totalRuns / 6 + (Math.random() - 0.5) * 20) })
    }
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      responseTimeChart.push({ time: time.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }), avg: Math.round(avg), p95: Math.round(avg * 1.5) })
    }
  ...
  const generateMockLogsAndAlerts = (status) => {
    const mockLogs = [
      { id: 1, level: "info", connection: status?.topConnections?.[0]?.connectionId || "API Connection", message: `Run completed successfully - ${status?.activity?.successfulRuns || 0} records processed`, timestamp: new Date(Date.now() - 300000) },
      { id: 2, level: status?.performance?.avgResponseTime > 300 ? "warning" : "info", connection: status?.topConnections?.[0]?.connectionId || "API Connection", message: `Response time: ${status?.performance?.avgResponseTime || 245}ms`, timestamp: new Date(Date.now() - 600000) },
      { id: 3, level: status?.activity?.failedRuns > 0 ? "error" : "info", connection: status?.topConnections?.[1]?.connectionId || "Weather API", message: status?.activity?.failedRuns > 0 ? "Connection timeout detected" : "Data transformation completed", timestamp: new Date(Date.now() - 900000) },
    ]
    setLogs(mockLogs)

    const mockAlerts = []
    if (status?.activity?.failedRuns > 0) {
      mockAlerts.push({ id: 1, severity: "high", connection: status?.topConnections?.[1]?.connectionId || "Weather API", message: `${status.activity.failedRuns} consecutive failures detected`, timestamp: new Date(Date.now() - 900000), status: "active" })
    }
    if (status?.performance?.avgResponseTime > 300) {
      mockAlerts.push({ id: 2, severity: "medium", connection: status?.topConnections?.[0]?.connectionId || "API Connection", message: "Average response time increased significantly", timestamp: new Date(Date.now() - 1800000), status: "active" })
    }
                      ...
                      <div key={log.id} className="flex items-start gap-3 p-3 rounded-lg hover:bg-muted/50">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <Badge variant="outline" className="text-xs">{log.connection}</Badge>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  {alerts.map((alert) => (
                    <div key={alert.id} className={`p-4 border rounded-lg ${alert.status === "resolved" ? "opacity-50" : ""}`}>
