...
export function EmbeddedDashboard({
  ...
  // Memoize params string
  const paramsString = useMemo(() => JSON.stringify(params || {}), [params])

  // Build full embed URL with parameters
  const buildEmbedUrl = (baseUrl, params) => {
    try {
        ...
        {!loading && embedUrl && (
          <div className="relative w-full" style={{ height }}>
            <iframe
              ...
              onError={() => {
                setError(
                  'Unable to embed report. This may be due to browser security restrictions or report sharing settings.'
                )
                setLoading(false)
              }}
            />
